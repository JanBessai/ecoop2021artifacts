    ...
    public interface TestTemplate extends M4Test.TestTemplate {
        default void test() {
           ...
           Exp neg1 = new Neg(lit1);
            ...
            org.junit.Assert.assertEquals(mult1.id(), new Mult(lit1, lit2).id());
            org.junit.Assert.assertEquals(divd1.id(), new Divd(lit1, lit2).id());
            org.junit.Assert.assertEquals(sub1.id(), new Sub(lit1, lit2).id());
            org.junit.Assert.assertEquals(add1.id(), new Add(lit1, lit2).id());
            org.junit.Assert.assertEquals(neg1.id(), new Neg(lit2).id());

            Node n1 = new Node(lit1.id(), new Leaf<>(1.0));
            Node n2 = new Node(lit2.id(),  new Leaf<>(2.0));
            util.Tree m3 = new Node(mult1.id(), n1, n2);
            org.junit.Assert.assertEquals(m3, new Mult(lit1, lit2).astree());

            util.Tree d3 = new Node(divd1.id(), n1, n2);
            org.junit.Assert.assertEquals(d3, new Divd(lit1, lit2).astree());

            util.Tree n3 = new Node(neg1.id(), n1);
            org.junit.Assert.assertEquals(n3, new Neg(lit1).astree());

            util.Tree a3 = new Node(add1.id(), n1, n2);
            org.junit.Assert.assertEquals(a3, new Add(lit1, lit2).astree());

            util.Tree s3 = new Node(sub1.id(), n1, n2);
            org.junit.Assert.assertEquals(s3, new Sub(lit1, lit2).astree());
            ...
            org.junit.Assert.assertTrue(new Mult(new Mult(new Lit(2.0), new Lit(1.0)), new Add(new Lit(0.0), new Lit(7.0))).simplify().astree().equals(new Node(2409808, new Node(76407, new Leaf<>(2.0)), new Node(76407, new Leaf<>(7.0)))));
        }
    }
