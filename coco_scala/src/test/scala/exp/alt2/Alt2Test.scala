...
import org.scalatest.FunSuite
    ...
    trait TestTemplate[FT] extends Factory[FT] with exp.alt1.TestTemplate[FT] {
      ...
      override def test() : Unit = {
        ...
        val mb = convert(pwr).multby(this.lit(4.0)); //mult by 4 just like raising by additional 2

        assert(32.0 === convert(pwr).eval)
        assert("(2.0^5.0)" === convert(pwr).prettyp)

        //based upon the CONTEXT (M7I2 or I2) you might get different results for mb
        assert("(2.0^(5.0+2.0))" === convert(mb).prettyp || "((2.0^5.0)*4.0)".equals(convert(mb).prettyp))
      }
    }
