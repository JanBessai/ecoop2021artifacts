...
trait TestTemplate[FT] extends Factory[FT] with exp.m2.TestTemplate[FT] {
  ...
  override def test() : Unit = {
    ...
    assert("(3.0*2.0)" === expr1.prettyp)

    val expr2 = this.divd(this.lit(3.0), this.lit(2.0))
    assert("(3.0/2.0)" === expr2.prettyp)

    val expr3 = this.neg(this.lit(3.0))
    assert("(-3.0)"===  expr3.prettyp)

    val expr4 = this.neg(expr1)
    assert("(-(3.0*2.0))"===  expr4.prettyp)

    assert(this.neg(this.lit(1.0)).eval === -1.0, 0.0)
    assert("(-1.0)"  === this.neg(this.lit(1.0)).prettyp)
    ...
    assert(this.neg(this.lit(5.0)).eval === -5.0)
    assert("(-(2.0*3.0))" === this.neg(this.mult(this.lit(2.0), this.lit(3.0))).prettyp)
  }
}
