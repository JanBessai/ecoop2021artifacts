...
public class I2Test {
    public interface TestTemplate<FT> extends coco.ep.i2.Factory<FT>, I1Test.TestTemplate<FT> {
        default void test() {
            ...
            Assertions.assertEquals(32.0, convert(pwr).eval(), 0.0);
            Assertions.assertEquals("(2.0^5.0)", convert(pwr).prettyp());

            //based upon the CONTEXT (M7I2 or I2) you might get different results for mb
            Assertions.assertTrue("(2.0^(5.0+2.0))".equals(convert(mb).prettyp()) ||
