...
public class M7i2Test {
    public interface TestTemplate<FT> extends coco.ep.m7i2.Factory<FT>, M7Test.TestTemplate<FT>, I2Test.TestTemplate<FT> {
        default void test() {
            ...
            Assert.assertFalse(convert(pwr).eql(mb));
            Assert.assertFalse(convert(mb).eql(pwr));

            Assert.assertTrue(convert(pwr).equals(pwr2));
            Assert.assertTrue(convert(pwr).eql(pwr2));
            Assert.assertTrue(convert(pwr2).eql(pwr));

            Exp<FT> neg = this.neg(this.lit(2.0));
            Exp<FT> neg2 = convert(neg).powby(this.lit(5.0));
            Assert.assertEquals(-32.0, convert(neg2).eval(), 0.0);

            Exp<FT> neg3 = convert(neg).multby(this.lit(5.0));
            Assert.assertEquals(-10.0, convert(neg3).eval(), 0.0);

            Exp<FT> divd2 = convert(this.divd(this.lit(6.0), lit(2.0))).multby(this.lit(5.0));
            Assert.assertEquals(15.0, convert(divd2).eval(), 0.0);

            Assert.assertEquals(40.0, convert(mb).eval(), 0.0);
            Assert.assertEquals("((2.0*5.0)*4.0)", convert(mb).prettyp());
            ...
            org.junit.Assert.assertEquals("1.0", convert(convert(expr2).simplify()).prettyp());

            coco.ep.Exp<FT> expr3 = this.power(this.lit(3.0), this.lit(2.0));
            org.junit.Assert.assertEquals("(3.0^2.0)", convert(convert(expr3).simplify()).prettyp());
            ...
            org.junit.Assert.assertEquals("(5.0^9.0)", convert(pwr1).prettyp());

            Assert.assertEquals(convert(this.power(this.lit(1.0), this.lit(12.0))).collect(), Arrays.asList(1.0, 12.0));

            Exp<FT> pwr3 = this.power(lit(2.0), lit(3.0));
            Exp<FT> pwr4 = convert(pwr3).powby(this.lit(4.0));
            org.junit.Assert.assertEquals("((2.0^3.0)^4.0)", convert(pwr4).prettyp());
        }
    }
