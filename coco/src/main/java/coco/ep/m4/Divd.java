...
public interface Divd<FT> extends coco.ep.m3.Divd<FT>, Exp<FT> {
    ...
    public default coco.ep.Exp<FT> simplify() {
        if (Double.valueOf(this.convert(this.getLeft()).eval()).equals(0.0)) {
            return this.lit(0.0);
        } else if (Double.valueOf(this.convert(this.getRight()).eval()).equals(1.0)) {
            return this.convert(this.getLeft()).simplify();
        } else if (Double.valueOf(this.convert(this.getLeft()).eval()).equals(this.convert(this.getRight()).eval())) {
            return this.lit(1.0);
        } else if (Double.valueOf(this.convert(this.getLeft()).eval()).equals(-1.0 * this.convert(this.getRight()).eval())) {
            return this.lit(-1.0);
        } else {
            return this.divd(this.convert(this.getLeft()).simplify(), this.convert(this.getRight()).simplify());
        }
    }
