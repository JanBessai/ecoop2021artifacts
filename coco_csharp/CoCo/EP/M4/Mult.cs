...
namespace EP.M4 {
    public interface Mult<FT> : Exp<FT>, EP.M3.Mult<FT> {
        ...
        EP.Exp<FT> Exp<FT>.simplify() {
            if (convert(getLeft()).eval() == (0.0) || convert(getRight()).eval() == (0.0)) {
                return lit(0.0);
                ...
                return mult(convert(getLeft()).simplify(), convert(getRight()).simplify());
            }
        }
